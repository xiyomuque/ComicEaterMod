...
} // had ,png here in the list as well
...
async function callTrimImage(imagePath, outputPath) {
  log.debug('callTrimImage called');
  return new Promise((resolve, reject) => gm(imagePath)
    // .despeckle()
    // .stegano()
    .fuzz('30%')
...
async function callNormalizeImage(imagePath, outputPath) {
  log.debug('callNormalizeImage called');
  return new Promise((resolve, reject) => gm(imagePath)
    .set('histogram-threshold', 3)
    .normalize()
    .write(outputPath, (err) => {
    // log.debug(`Validating Image: ${tmp}`); TODO enable as log.silly
      if (!err) {
        ...
        log.debug(`Image Normalization resulted in '${err}'`);
        reject(err);
      }
    }));
}
...
async function normalizeImages(extractedArchiveDir) {
  log.info(`Normalizing images in "${extractedArchiveDir}"`);
  const imagePaths = await getImages(extractedArchiveDir);
  await Promise.all(
    imagePaths.map(
      async (imagePath) => LIMIT(async () => {
        try {
          await callNormalizeImage(imagePath, imagePath);
        } catch (e) {
          throw new Error(e);
        }
      }),
  ...
  log.info(`Finished normalizing images in "${extractedArchiveDir}"`);
}
...
async function upscaleImageDir(imageDir) {
  const wrappedInput = wrapInQuotes(imageDir);
  return UPSCALE_LIMIT(
    // async () => callProgram('realcugan-ncnn-vulkan', ['-i', wrappedInput, '-o', wrappedInput, '-n', '2', '-s', '2', '-f', 'webp']),
    async () => callProgram('waifu2x-ncnn-vulkan', ['-i', wrappedInput, '-o', wrappedInput, '-n', '2', '-s', '2', '-f', 'webp']),
  );
}
...
async function upscaleImages(extractedArchiveDir) {
  ...
  if (upscaledImages.length !== imagePaths.length) { //it was !==
